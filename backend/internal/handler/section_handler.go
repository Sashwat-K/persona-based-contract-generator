...
func (h *SectionHandler) SubmitSection(w http.ResponseWriter, r *http.Request) {
	...
	if !role.IsValid() {
		writeError(w, model.ErrInvalidRequest("Invalid persona role."))
		return
	}

	// Double-check the authorized user actually has the role they are claiming to submit as
	if !middleware.HasRole(r.Context(), req.PersonaRole) {
		writeError(w, model.ErrForbidden("You do not have the "+req.PersonaRole+" role."))
		return
	}
